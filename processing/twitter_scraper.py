...
def scrape_twitter_feeds():
    ...
    if not get_bearer_token():
        print("INFO: X_BEARER_TOKEN not set — skipping Twitter/X scrape.")
        print("\n" + "="*60)
        print("HOW TO GET A FREE X/TWITTER API BEARER TOKEN")
        print("="*60)
        print()
        print("The X API v2 Free tier includes GET /2/tweets/search/recent")
        print("which is all HEAT needs. No paid plan required.")
        print()
        print("Steps:")
        print("  1. Go to https://developer.x.com and sign in")
        print("  2. Navigate to the Developer Portal → Projects & Apps")
        print("  3. Click 'Create Project' → name it (e.g. 'HEAT Civic Signal')")
        print("  4. Select 'Free' access level when prompted")
        print("  5. Create an App inside the project")
        print("  6. Go to 'Keys and Tokens' tab")
        print("  7. Under 'Bearer Token', click 'Generate'")
        print("  8. Copy the token and set the env variable:")
        print()
        print("   Windows:  set X_BEARER_TOKEN=your_token_here")
        print("   Linux:    export X_BEARER_TOKEN='your_token_here'")
        print("   .env:     X_BEARER_TOKEN=your_token_here")
        print("   GitHub:   Settings → Secrets → TWITTER_BEARER_TOKEN")
        print()
        print("Free tier limits (sufficient for HEAT):")
        print("  • 1 App environment")
        print("  • GET /2/tweets/search/recent (last 7 days)")
        print("  • 10,000 tweet reads / month")
        print("  • No paid subscription required")
        print("="*60 + "\n")
        return
