...
```
heat/
├── build/                      # Static files (deploy to S3)
│   ├── index.html
│   ├── app.js
│   ├── styles.css
│   └── data/
│       ├── clusters.json       # Generated by pipeline
│       ├── timeline.json       # Generated by pipeline  
│       ├── keywords.json       # Generated by NLP analysis
│       ├── heatmap.json        # Generated by KDE
│       └── plainfield.geojson  # ZIP boundaries
│
├── processing/                 # Python data pipeline
│   ├── requirements.txt
│   ├── ingest.py              # Normalize raw CSVs
│   ├── cluster.py             # Semantic clustering (HDBSCAN)
│   ├── nlp_analysis.py        # Keywords + burst detection + trends
│   ├── heatmap.py             # Kernel density estimation
│   ├── scraper.py             # Ethical RSS/news scraping
│   ├── buffer.py              # Safety thresholds
│   └── export_static.py       # Export to JSON
│
├── data/
│   ├── raw/                   # Your input CSVs
│   │   ├── news.csv
...
│   ├── deploy_s3.bat          # Deploy to AWS S3
│   └── setup_s3.bat           # One-time S3 bucket setup
│
├── docs/
│   ├── POSITIONING.md         # Patent landscape analysis
...
- `clusters.json` — Semantic clusters with strength scores
- `timeline.json` — Weekly aggregation of all signals
...
## ☁️ Deploy to AWS S3 (Free)

### One-Time Setup

1. Install AWS CLI: https://aws.amazon.com/cli/
2. Configure credentials: `aws configure`
3. Edit `scripts/setup_s3.bat` — change `BUCKET_NAME`
...
❌ Real-time locations or alerts  
❌ Individual sightings or reports  
❌ Vehicle descriptions or routes  
...
```
Input texts → Embeddings (384-dim) → HDBSCAN → Semantic clusters
```
